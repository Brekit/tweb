...
export class AppMessageHistoryManager extends AppManager {
  ...
  private onMessageEdit = ({message, peerId, mid}: {message: MyMessage, peerId: PeerId, mid: number, storageKey: string}) => {
    ...
    this.recordMessageHistory(peerId, mid, MessageHistoryAction.EDITED, {
      editedMessage: copy(message),
      changes
    });

    this.extractAndStoreMedia(message, peerId);
  };
